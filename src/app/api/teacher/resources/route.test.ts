...
describe("POST /api/teacher/resources", () => {
  ...
  it("creates a video link and stores the pasted URL verbatim", async () => {
    requireAuth.mockResolvedValueOnce(TEACHER);
    // First select is the caller's teaching scope; second is the course-exists check.
    stubSelect(db, [[{ courseId: 3, jambSubjectId: null }], [{ id: 3 }]]);
  ...
  it("returns 404 when the scoped course does not exist", async () => {
    requireAuth.mockResolvedValueOnce(TEACHER);
    // The teacher must *teach* course 999 for the handler to reach the existence check.
    stubSelect(db, [[{ courseId: 999, jambSubjectId: null }], []]);
  ...
  it("returns 404 when the scoped JAMB subject does not exist", async () => {
    requireAuth.mockResolvedValueOnce(TEACHER);
    // The teacher must *teach* JAMB subject 999 for the handler to reach the existence check.
    stubSelect(db, [[{ courseId: null, jambSubjectId: 999 }], []]);
