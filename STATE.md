...
- [x] P2-2 — Teacher/Student/Aspirant route group shells
  ...
  - 2026-09-15: admins got their own authoring pages in the admin shell — `/admin/topics`, `/admin/questions`, `/admin/quizzes` (+ `[id]` builder) — rendering the same views as teachers via `components/teaching/` (the APIs were already admin+teacher guarded). The admin nav no longer links into `/teacher/*`, so an admin managing quizzes stays in the admin shell; admin dashboard gained a "Build a Quiz" quick action.
...
- [x] P3-3 — `/teacher/questions` editor UI
  ...
  - 2026-09-15: Bulk add gained a Fill-in-gap/Options type toggle (options rows: one per Tab column, correct one prefixed `*`; parser in `lib/questions/bulk-rows.ts`, 15 tests), a Tab-inserts-column-separator fix plus an "Insert separator" button for phones, and Chemistry/Physics/Mathematics symbol palettes that insert at the caret.
...
- [ ] P3-7 — `/teacher/results` + `/teacher/results/[quizId]` (Teacher results view)
  - **UNCHECKED ON PURPOSE — partial verification, not unwritten code.** Code is complete and the colocated tests pass (14/14, `pnpm vitest run src/app/api/teacher/results`). `pnpm lint`, `pnpm typecheck` and the full `pnpm test` suite could **not** be run in this session — the sandbox command classifier was unavailable for the whole session. Per this file's own rule ("a checked box that doesn't actually work is worse than an honest unchecked one"), the box stays unticked until those three are green. Tick it then; don't rebuild it.
  - Built to close Board feedback 2026-09-17 ("Staff Section: clicking the lower section options yields errors"): the teacher nav's last item, `Results` → `/teacher/results`, had no page behind it, so every teacher who tapped it got the 404 shell. The screen was specced in `.agents/design/screens-teacher.md` but never built — fixed by building it rather than dropping the nav item.
  - API: `GET /api/teacher/results` (the caller's quizzes with ≥1 submitted attempt + released/held tallies; tallied as flat rows counted in JS, same approach as the published-quiz counts on `/api/admin/teachers`) and `GET /api/teacher/results/[quizId]` (stats + one row per person who sat it). Both `requireAuth(["admin","teacher"])` + `ownershipScope`, so teachers see only their own quizzes and admins bypass ownership. Colocated tests (14): 401, 403, 400 bad id, 404 missing, 404 another teacher's quiz, admin access, held-score suppression, null-average-before-release, null score tolerated.
  - **Held-score rule (Board decision 2026-09-17, resolving spec-vs-`DESIGN.md` conflict):** the spec wireframe showed a held attempt's score next to a `Held` pill; `DESIGN.md` §4 forbids sending a held raw score to a non-admin client. `DESIGN.md` won — a teacher sees *that* an attempt is pending, never its mark. Enforced in the query itself: the held-attempts select doesn't project `score`, so an unreleased mark is never read out of the DB. Consequence: `avgScore` / `passRate` are over released attempts only and are `null` (rendered "—") until something is released; the `Attempts` stat is a count, not a score, so a teacher can still see responses exist.
  - FLAGGED (AGENTS.md §6): `screens-teacher.md` says a quiz owned by another teacher answers **403**; this returns **404 "Quiz not found"**, matching the existing convention in `/api/teacher/quizzes/[id]` so the route doesn't confirm another teacher's quiz exists. Tighten only if the Board wants the distinction exposed.
  - UI: shared `components/teaching/results-view.tsx` + `results-detail-view.tsx` (`basePath` prop, matching the quizzes/builder views so a future admin-shell twin is additive). Per-person rows render as a list, not a table — the spec's own wireframe is `LIST ROW` format and a 4-column table is wrong on phones. Loading/error+Retry/empty/held-note states all handled; pills carry words, not colour alone.
